...
from data_engineering.load.model.titanic_train import TitanicTrain
from data_engineering.load.repository.titanic_repository import TitanicRepository
...
class TitanicTrainSqlRepository(TitanicRepository):
    ...
    def bulk_load_data(self, data: List):
        titanic_train_data = [TitanicTrain(passenger_id=int(row[0]),
                                           ...
                                           fare=float(row[9]),
                                           cabin=row[10],
                                           embarked=row[11]) for row in data]
